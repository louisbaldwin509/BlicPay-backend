...
// ---- Reset done TÈS anvan pase LIVE ----
//
// Efase TOUT done finansye/tès (depo, retrè, transfè, prè, objektif, pòch,
// adezyon/kotizasyon Sòl, notifikasyon) epi remèt balans tout kont ak
// wotasyon chak gwoup Sòl a zewo. KONT itilizatè yo (non, telefòn, modpas,
// KYC, wòl) rete ANTYE — sa a se yon POZ, pa yon "efase tout".
//
// Pwoteje pa yon mo konfimasyon egzat nan kò demand lan, pou anpeche yon
// deklanchman aksidantèl (egzanp: yon script oswa yon lòt admin ki fè yon
// tès sou wout sa a san reflechi).
adminRouter.post('/system/reset-test-data', requireAdmin, async (req, res) => {
  if (req.body?.confirm !== 'REMET_TOUT_A_ZERO') {
    return res.status(400).json({
      error: 'Konfimasyon obligatwa. Voye { "confirm": "REMET_TOUT_A_ZERO" } nan kò demand lan pou kontinye.',
    });
  }
  ...
  const result = await prisma.$transaction([
    ...
    prisma.solContribution.deleteMany({}),
    prisma.solDocument.deleteMany({}),
    prisma.solMembership.deleteMany({}),
    prisma.deposit.deleteMany({}),
    prisma.withdrawal.deleteMany({}),
    prisma.transfer.deleteMany({}),
    prisma.savingsGoal.deleteMany({}),
    prisma.pocket.deleteMany({}),
    prisma.notification.deleteMany({}),
    prisma.solGroup.updateMany({ data: { currentTurn: 0, startedAt: null, completedAt: null, reserveBalance: 0 } }),
    prisma.user.updateMany({ data: { balance: 0, feeableBalance: 0, solPayoutBalance: 0 } }),
  ]);

  console.log(`[RESET] ${req.user.id} remèt tout done finansye/tès a zewo — ${new Date().toISOString()}`);

  res.json({
    ok: true,
    deleted: {
      loanInstallments: result[0].count,
      loans: result[1].count,
      solContributions: result[2].count,
      solDocuments: result[3].count,
      solMemberships: result[4].count,
      deposits: result[5].count,
      withdrawals: result[6].count,
      transfers: result[7].count,
      savingsGoals: result[8].count,
      pockets: result[9].count,
      notifications: result[10].count,
    },
    solGroupsReset: result[11].count,
    usersReset: result[12].count,
  });
});
