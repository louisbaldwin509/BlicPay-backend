...
adminRouter.get('/sol/requests/pending', async (req, res) => {
  const requests = await prisma.solMembership.findMany({
    ...
    include: {
      ...
      group: { select: { name: true, tier: true, frequency: true, amount: true, maxMembers: true } },
    },
  });
  res.json({ requests });
});
...
adminRouter.post('/sol/requests/:id/approve', async (req, res) => {
  ...
  const approvedCount = await prisma.solMembership.count({ where: { groupId: membership.groupId, status: 'approved' } });
  if (approvedCount >= membership.group.maxMembers) {
  ...
  const updated = await prisma.solMembership.update({
    ...
    data: { status: 'approved', turnIndex: approvedCount, decidedAt: new Date(), decidedBy: req.user.id },
  });
